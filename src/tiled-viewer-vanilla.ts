...
(function () {
  ...
  function resolvePath(base: string, rel: string): string {
    ...
    if (m) result = '/' + m[1] + '/' + result.substring(m[0].length);
    return result;
  }
  ...
  function drawMap() {
    ...
    for (let li = 0; li < mapData.layers.length; li++) {
      ...
      for (let row = 0; row < l.height; row++) {
        for (let col = 0; col < l.width; col++) {
          ...
          if (gid === 0) { gid0++; continue; }

          let bestTs: any = null, bestGid = 0;
          for (const ts of mapData.tilesets) {
            if (ts.firstgid <= gid && ts.firstgid > bestGid) { bestTs = ts; bestGid = ts.firstgid; }
          }
          if (!bestTs || !bestTs.imageElement) { noImg++; continue; }
          ...
          ctx.drawImage(bestTs.imageElement,
            sx, sy, bestTs.tilewidth, bestTs.tileheight,
            col * mapData.tilewidth + (l.x || 0),
            row * mapData.tileheight + (l.y || 0),
    ...
    if (st) {
      const tsInfo = mapData.tilesets.map((t: any) => (t.name || 'ts') + ':' + (t.imageElement ? '✓' : '✗')).join(', ');
  ...
  async function loadMap(idx: number) {
    ...
    for (const ts of map.tilesets) {
      ...
      if (imgPath) {
        try {
          ...
          if (url) {
            ts.imageElement = await new Promise<HTMLImageElement>((res, rej) => {
    ...
    mapData = map;
    currentIdx = idx;
    visible = map.layers.map((l: any) => l.visible);

    // Update toolbar file name
    const fn = document.querySelector('#tv-filename');
    if (fn) fn.textContent = maps[idx].name;
  ...
  function updateMapButtons() {
    ...
    el.innerHTML = maps.map((m, i) =>
      `<button class="tv-map-btn${i === currentIdx ? ' tv-active' : ''}" data-idx="${i}">${m.name}</button>`
    ).join('');
  ...
  function updateLayers() {
    ...
    el.innerHTML = mapData.layers.map((l: any, i: number) =>
      `<label class="tv-layer${visible[i] ? '' : ' tv-off'}">
    ...
    el.querySelectorAll('input').forEach(cb => {
      cb.addEventListener('change', () => {
        const i = parseInt((cb as HTMLElement).dataset.idx!);
        visible[i] = !visible[i];
  ...
  async function browse() {
    ...
    if (!list) return;
    const jsonFiles = list.files.filter((x: string) => x.toLowerCase().endsWith('.json'));
    maps = jsonFiles.map((x: string) => ({ name: x, path: resolvePath(f, x) }));

    const fp = document.querySelector('#tv-path');
    if (fp) fp.textContent = f;

    if (maps.length) await loadMap(0);
  }
  ...
  (window as any).__tiledViewerMount = mount;
})();
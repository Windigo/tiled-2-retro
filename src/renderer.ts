...
function buildMapsAb3(): string {
  ...
  for (let m = 0; m < numMaps; m++) {
    ...
    for (let r = 0; r < mRows; r++) { for (let c = 0; c < mCols; c++) { allGridVals.push(map[r]?.[c] ?? 0); } }
  }
  const gridDataLines: string[] = [];
  for (let i = 0; i < allGridVals.length; i += 16) { gridDataLines.push(`Data.w ${allGridVals.slice(i, i + 16).join(',')}`); }

  const flagVals: number[] = [];
  for (let i = 0; i < maxTiles; i++) flagVals.push(toSignedWord(tileFlags[i] ?? 0));
...
function handleMenuAction(action: string): void {
  switch (action) {
    ...
    case 'save':
      ...
      break;
    case 'settings':
      if (!projectLoaded) { showToast('No project loaded', 'error'); return; }
      showProjectSettings();
      break;
    case 'export':
      switchTab('tiled-viewer');
...
// ─── Tiled Viewer Amiga export ─────────────────────────────────────────────

interface TiledExportData {
  mapJson: any;
  pngPath: string;
...
interface TiledLayerMeta {
  name: string;
  index: number;
  collision: number;  // 0=none, 1=solid, 2=pickup, 3=hazard
  tileData: number[];
  width: number;
  height: number;
}

function buildTiledIff(img: HTMLImageElement, nPlanes: number): Uint8Array {
  const w = img.width;
  const h = img.height;
  const bytesPerRow = ((w + 15) >> 4) << 1;
  const maxColors = 1 << nPlanes;
...
function parseTiledLayers(mapJson: any): TiledLayerMeta[] {
  const layers: TiledLayerMeta[] = [];
    ...
    if (l.properties) {
      for (const prop of l.properties) {
        if (prop.name === 'collision' || prop.name === 'col') {
          const v = prop.value;
          if (typeof v === 'number') collision = v;
          else if (typeof v === 'string') {
            const lower = v.toLowerCase();
            if (lower === 'solid' || lower === 'wall' || lower === 'platform') collision = 1;
            else if (lower === 'pickup' || lower === 'item' || lower === 'collectible') collision = 2;
            else if (lower === 'hazard' || lower === 'enemy' || lower === 'spike') collision = 3;
            else collision = parseInt(v) || 0;
          }
        }
      }
    }
    // Guess collision from layer name as fallback
    if (collision === 0 && (l.name || '').toLowerCase().includes('solid')) collision = 1;
    if (collision === 0 && (l.name || '').toLowerCase().includes('wall')) collision = 1;
    if (collision === 0 && (l.name || '').toLowerCase().includes('pickup')) collision = 2;
    if (collision === 0 && (l.name || '').toLowerCase().includes('hazard')) collision = 3;
    if (collision === 0 && (l.name || '').toLowerCase().includes('spike')) collision = 3;
    if (collision === 0 && (l.name || '').toLowerCase().includes('enemy')) collision = 3;
...
function buildTiledMapAb3(mapJson: any, imageWidth?: number, imageHeight?: number): string {
  ...
  const firstgidList = (mapJson.tilesets || []).map((ts: any) => ts.firstgid || 1);
  function gidToTile(gid: number): number {
    if (!gid) return 0;
    let best = 0;
    for (const fg of firstgidList) {
      if (fg <= gid && fg > best) best = fg;
    }
...
function buildTiledGameAb3(layers: TiledLayerMeta[], mapJson: any, bp: number, imageWidth?: number, imageHeight?: number): string {
  return buildTiledMapAb3(mapJson, imageWidth, imageHeight);
}

function buildTiledPlayerAb3(): string {
  ...
  async function loadMap(idx: number) {
    ...
    if (!pngPath) {
      ...
      if (!pngPath) {
        ...
        if (ts?.image) pngPath = cleanPath(ts.image);
        else if (ts?.source) {
          try {

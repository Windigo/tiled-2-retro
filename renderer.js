...
function buildGameAb3(mapJsons, bitplanes, spriteTileId, imageWidth, imageHeight) {
...
function buildMapdataAb3(mapJsons) {
    const sections = [];
    sections.push(`; ---------------------------------------------------------------`);
    ...
    for (let lvl = 0; lvl < mapJsons.length; lvl++) {
        ...
        const allTiles = new Array(cells).fill(-1);
        for (const layer of mapJson.layers) {
            if (layer.type !== 'tilelayer' || !layer.data)
                continue;
            for (let i = 0; i < cells; i++) {
...
function initTiledViewerTab() {
    ...
    async function doTiledExport() {
        ...
        if (allMapJsons.length === 0) {
            showToast('No level data found', 'error');
            return;
        }
        ...
        const gameAb3Bytes = stringToAmigaBytes(buildGameAb3(allMapJsons, bp, 459, imgW, imgH));
        if (!currentFolder) {
